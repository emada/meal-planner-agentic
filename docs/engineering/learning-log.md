...
Entries below marked `propose` and not yet in a contract pull request: **19**.
...
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------ | ---------------------------------------------------------------- | ---------------------------------------------------------------------------- | ------------------ |
| A1  | Two commit messages described work that did not exist: a widened selector and a fixture. A scripted find-and-replace had matched nothing and exited successfully.        | review, twice (`e68b7ba`, `21e47d7`) | verify the replacement applied, then read the file back          | none — the failure is in the tool, and only its consequence reaches a commit | propose            |
| A2  | The gate register said AC13 was not exercised, and that the app rendered an empty `<main>`, after both had stopped being true.                                           | review                               | replaced with the three tests that assert it                     | `governance-consistency.test.ts` rejects a gate pointed at a shipped slice   | propose            |
| A3  | Three threat-model rows stayed `Planned — S3` after S3 shipped.                                                                                                          | review                               | marked `Adopted`                                                 | the same guard, widened to any dash and to a parenthesised slice             | propose            |
| A4  | The gate-count sentence said "eleven of twenty" after the table had grown. Three rounds of this.                                                                         | review, three times                  | counts derived from the table by test                            | `governance-consistency.test.ts`                                             | promoted-candidate |
...
| A8  | Correcting A7, the table was recomputed and the prose quoting it was not — the document contradicted itself.                                                             | review                               | the prose figures asserted by value, not only the table          | the same test                                                                | propose            |
...
Every one is a category no test in this repository covers. That is the honest
summary of what the gates are for and what they are not.
