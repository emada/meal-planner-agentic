...
**<!-- derived:pending-promotion -->4<!-- /derived -->**.

---

## The recurring classes

Twenty-eight entries, and they are not twenty-eight different mistakes. Three
...
| B10 | A completeness guard read too much, twice: at file scope one item was shielded by a sentence two sections away; rescoped to the section, the next was shielded by the sentence directly beneath the list. Green both times. | review, twice | scoped to the paragraph carrying the claim, then every item asserted load-bearing one at a time | `governance-consistency.test.ts` deletes each item in turn and requires the check to notice | propose  | —                                                           |
...
its evidence live in the same file. Choosing that span is a judgement, and it
was wrong twice running here. What ended it was not a better span but a
different assertion — remove each item and require the check to notice, so the
guard is measured item by item instead of in aggregate.
