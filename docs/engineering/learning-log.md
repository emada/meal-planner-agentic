...
| A10 | A count was bound to a copy of itself: the gate register and the roadmap were asserted to agree on how many gates are proven at the tool, and neither was read from the table holding the fact. Closing the gap would have left both false and the suite green — and the comment beside it called both counts derived. | review                               | derived from the probe cells, and every wording of the other count checked wherever it is written | `governance-consistency.test.ts`, probed by closing the row and by opening a second                     | promoted | `02-quality/06-claim-accuracy.md`        |
| A11 | The span fix in A10's commit was pinned for one arrangement and described as covering all of them. Prose reflowed onto the list's own line still widened it, and both the comment and the message said otherwise. Four inferred spans in a row were each defeated by ordinary editing.                                 | review                               | the span marked in the document with HTML comments instead of inferred from its punctuation       | `governance-consistency.test.ts`, probed by removing either marker and by dropping the list's full stop | promoted | `02-quality/06-claim-accuracy.md`        |
...
| B10 | A completeness guard read too much, three times: at file scope one item was shielded by a sentence two sections away; at section scope, by the sentence beneath the list; at paragraph scope, by that same sentence after a reflow. Green each time. | review, three times | the span narrowed to the enumeration itself — the sentence carrying the claim, not the prose around it | `governance-consistency.test.ts` pins the span against a fixture where the item is absent from the list and present beside it | promoted | `02-quality/06-claim-accuracy.md`                           |
| B11 | A single-file exemption was written as a phrase match over every document, so any later line could silence the guard by quoting it.                                                                                                                  | review              | bound to the one path and the one shape                                                                | the exemption count is asserted to be exactly one                                                                             | promoted | `02-quality/04-control-effectiveness.md`                    |
| B12 | The fix for B10 was asserted by deleting each item from the haystack and requiring the check to notice — true by construction, so it never failed. It was credited in this log and in a commit message with closing the class.                       | review              | deleted; the span is pinned by fixture instead                                                         | the fixture fails if the span widens                                                                                          | promoted | `02-quality/04-control-effectiveness.md`                    |
...
| D4  | The check added to verify a promoted entry read `.ai-engineering/.bootstrap`, which is a private submodule. It passed locally and failed on all twenty rows in CI, where the checkout has the product and not the engine.                                                                                                    | gate                                   | promoted | `02-quality/02-layered-gates.md`                                 |
| D5  | A negative probe was cleaned up with `git checkout -- <file>`, which reverted the uncommitted work in that file along with the probe. Four edits were lost and had to be rewritten; nothing warned, because discarding is what the command does.                                                                             | probe — it failed for the wrong reason | promoted | `02-quality/04-control-effectiveness.md`                         |
| D6  | The secret-scan gate failed for a reason unrelated to secrets: `gitleaks-action@v2` validates a licence over the network, could not reach `api.github.com`, and failed closed. A re-run passed. The one gate whose wiring this repository records as unprobed now has an observed failure mode, and it is not about secrets. | gate                                   | promoted | `02-quality/04-control-effectiveness.md`                         |
