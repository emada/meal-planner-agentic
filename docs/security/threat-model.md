...
Scope: the static client application defined in `SPEC.md`. Last reviewed at S0 on 2026-08-09, revised 2026-08-10 after semantic review, and again on 2026-08-11 when the product was complete. Review again when a new external service, any server-side component, or any form of user-supplied content is introduced.
...
| T5  | Corrupt, oversized, or attacker-planted `localStorage` content crashing the app          | Medium     | Low    | Schema validation on read, safe recovery, write-failure handling                                                                                                                                                                                           | Adopted (S3)                    |
| T6  | Malformed or hostile API response shape                                                  | Medium     | Medium | Zod parsing at the `api/` boundary; unparsed data never reaches `ui/`                                                                                                                                                                                      | Adopted (S1)                    |
| T7  | TheMealDB unavailable, throttled, or slow                                                | High       | Low    | Visible error state with retry; treated as an acceptance criterion, not polish                                                                                                                                                                             | Adopted (S1)                    |
...
- **TheMealDB test key (spec O1). Accepted by the human on 2026-08-10.** Key `1` is rate-limited, carries no availability commitment, and is documented for development rather than production. The application now serves real users on it. Consequences accepted: the recipe search can throttle or fail for reasons outside our control, and there is no supported channel to appeal to when it does. Mitigation is limited to degrading visibly — the AC3 error state with retry — which is a gate, not a fix. Revisit by obtaining a supported key; adding caching or a proxy would contradict the no-backend non-goal and requires a spec revision.
  ...
  **The exposure grew at S8 (2026-08-11) and the acceptance predates it.** Until then no request reached TheMealDB until the user acted, so a visitor who arrived and left cost nothing. The category browser loads on arrival, so every page view now spends at least one request against a rate-limited key — before AC15 the floor was zero. Nothing new is sent and no new party is involved, which is why `docs/privacy/assessment.md` records this as a timing change rather than a data change; but a rate limit is consumed per visit now, not per search. **Re-accepted on 2026-08-12** on that basis: the human was shown the change of shape — a rate limit spent per page view rather than per search — and accepted it. `ROADMAP.md` records what retiring the key would take.
