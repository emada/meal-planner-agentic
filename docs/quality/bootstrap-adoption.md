...
| Control effectiveness     | Partial | Twenty of twenty-one mandatory gates have a recorded negative probe; the other one remains unproven. Per-gate status and `Last proven` dates are in `docs/quality/gates.md` |
...
| Before the first merge        | **Partial.** Local and CI gates green, ruleset active on `main`, Vercel git integration connected and verified. One checklist item is unmet and knowingly carried into this merge: one of twenty-one mandatory gates has no negative probe, named with its reason in `docs/quality/gates.md`. Separately, `SWEAI Review / Claude` became a required context on 2026-08-10, so a merge with no review is refused. The remaining unmet item is negative-probe coverage, accepted by the human when autonomy was raised |
