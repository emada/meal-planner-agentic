...
| Unit/integration tests              | Vitest + Testing Library                                                                                                                                 | push, CI                                                                                                                                           | **none recorded** — not yet proven                                                                                                         | —           |
| Submodule installation integrity    | Vitest asserts `.ai-engineering` stays Git mode `160000` with no vendored or product-owned contract copy                                                 | push, CI                                                                                                                                           | **none recorded** — asserted but not yet disproven                                                                                         | —           |
| Secret scan, full history           | gitleaks (CI)                                                                                                                                            | CI                                                                                                                                                 | **none recorded** — not yet proven                                                                                                         | —           |
| Reproducible build                  | `tsc --noEmit` + `vite build` at pre-push; additionally `npm ci` from a clean checkout in CI                                                             | push, CI                                                                                                                                           | **none recorded** — not yet proven                                                                                                         | —           |
| Browser journeys, mobile + desktop  | Playwright, `desktop-chromium` and `mobile-chromium`                                                                                                     | CI                                                                                                                                                 | browse skipped its detail lookup and opened the modal on partial data → 6 journeys failed across both projects; 20/20 pass once reverted   | 2026-08-11  |
...
| No console errors on load           | Playwright assertion                                                                                                                                     | CI                                                                                                                                                 | **none recorded** — not yet proven                                                                                                         | —           |
| Dependency scan                     | `npm audit --audit-level=high` + OSV-Scanner over the lockfile                                                                                           | CI                                                                                                                                                 | **none recorded** — pulled forward 2026-08-10, not yet proven                                                                              | —           |
| Static analysis                     | CodeQL `security-extended`, required as both the job and the `CodeQL` result check                                                                       | CI                                                                                                                                                 | **none recorded** — pulled forward 2026-08-10, not yet proven                                                                              | —           |
...
| Automated accessibility             | axe-core through Playwright over eight states: search, categories, category results, results, modal, shopping list, confirmation, and both error regions | CI                                                                                                                                                 | contrast dropped below AA → axe reports a `color-contrast` violation and the run fails                                                     | 2026-08-11  |
...
**Thirteen of twenty-one mandatory gates are proven; eight are not.** The unproven eight are configured and green but have never demonstrated rejection, which is stated here rather than presented as verified. Most are structurally hard to probe locally — a deployment, a full-history scan, a required context published by CI. They are listed so a green board is not read as a proven one. The pre-push layer additionally has no probe showing it rejects a failing build or test; that is a layer, not a table row.
...
- **It records that gates ran and what they returned, not that they work.** A passing run over an ineffective gate is still a failing control. Effectiveness is the negative-probe cycle above, and eight of twenty-one gates still have no probe.
...
Run against the live site rather than a preview, with the real TheMealDB, on 2026-08-11:
...
**This is a dated manual observation, not derived evidence.** It was run from a throwaway Playwright project outside this repository — the committed config points `baseURL` at a local preview server — so nothing here re-runs it, and no artifact is retained. Read it as someone having checked, not as something a machine will check again. That is the opposite of how the gate evidence above works, and the distinction matters more than the result.

Removing an item and clearing the list were **not** smoke-checked. They are device-local, they touch no external service, and they are covered at both the unit and browser level, so the live run would only re-test what a stub already proves.
