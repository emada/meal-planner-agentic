...
| Secret scan, full history           | gitleaks (CI)                                                                                                                                                                                          | CI                                                                                                                                                 | a `ghp_`-shaped token committed in a throwaway clone → `leaks found: 1`, exit 1; clean again once reverted                                                             | 2026-08-11  |
...
| Dependency scan                     | `npm audit --audit-level=high` + OSV-Scanner over the lockfile                                                                                                                                         | CI                                                                                                                                                 | `lodash@4.17.20` (GHSA-xxjr-mmjv-4gpg, high) added to the lockfile → `npm audit --audit-level=high` exit 1; exit 0 on the real lockfile                                | 2026-08-11  |
...
Two notes worth more than the count:
...
- The static-analysis probe is not synthetic. CodeQL rejected code written for this project — a file-system race in `scripts/check-bundle-size.mjs`, found at S6 — and it was fixed rather than suppressed.
...
| The deployment serves the four documented security headers              | pass   |

Fourteen runs: seven journeys across a 375px and a desktop viewport.
