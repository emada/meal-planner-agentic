...
| Dependency scan                     | `npm audit --audit-level=high` + OSV-Scanner over the lockfile                                                                                                                                                                                                                                                         | CI                                                                                                                                                 | `lodash@4.17.20` added to a scratch lockfile → `npm audit --audit-level=high` exit 1 and OSV-Scanner 2.5.0 exit 1 reporting 5 advisories including GHSA-35jh-r3h4-6jhm (CVSS 8.1); both exit 0 on the real lockfile             | 2026-08-11  |
...
| Preview deployment                  | Vercel git integration, `Vercel` status context                                                                                                                                                                                                                                                                        | CI (required context on `main`)                                                                                                                    | a type error pushed on a throwaway branch (PR #14) → `Vercel` reported failure, `mergeStateStatus` became `BLOCKED`, and `gh pr merge` was refused with `the base branch policy prohibits the merge`; PR closed, branch deleted | 2026-08-11  |
| Automated accessibility             | axe-core through Playwright over eight scans: the idle view with its category grid, a category's results, search results, the modal, the shopping list, the clear confirmation, and both error regions. Target set to WCAG 2.1 AA on 2026-08-11 — see `docs/quality/accessibility.md` for what the scan does not cover | CI                                                                                                                                                 | contrast dropped below AA → axe reports a `color-contrast` violation and the run fails                                                                                                                                          | 2026-08-11  |
| Reflow at 320px (WCAG 1.4.10)       | Playwright `reflow-320` project over five journeys                                                                                                                                                                                                                                                                     | CI                                                                                                                                                 | `overflow-wrap: anywhere` removed from `.card__title` → 3 of 5 journeys failed on a clipped title; removed from `.ingredients__name` → the modal journey failed; 5/5 pass once restored                                         | 2026-08-11  |
...
**All twenty-two mandatory gates have a recorded negative probe.** Seven were added at S9 in a throwaway clone or a scratch lockfile, so no credential-shaped string, broken submodule, or vulnerable dependency ever entered this repository's history.

The last one, preview deployment, was argued here to be not worth proving — that it would mean putting a broken deployment into the project's Vercel history to demonstrate what the deployment logs already show on every push. **The human challenged that on 2026-08-11 and was right.** A preview build is not production: production deploys only from `main`, so a failed preview on a branch that never merges costs one entry in a dashboard. And the deployment logs show deployments _succeeding_; they say nothing about whether a failure produces a red required context that refuses the merge. Those are different claims, and only the second is what the gate exists for. The probe took eight minutes.
